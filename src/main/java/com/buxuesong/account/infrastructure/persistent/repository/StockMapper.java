...
public interface StockMapper {
    ...
    @Select({
        "<script> select CODE, NAME, COST_PRICE costPrise, BONDS, APP, HIDE from STOCK WHERE 1=1 AND USERNAME = #{username} <if test=\"app!=null and app!=''\"> and APP = #{app} </if> order by APP ASC, substr(code,3,6) ASC </script>" })
    List<StockPO> findAllStock(@Param("app") String app, @Param("username") String username);

    @Select("select CODE, NAME, COST_PRICE costPrise, BONDS, APP, HIDE from STOCK where code = #{code} AND USERNAME = #{username} ")
