...
public interface FundJZMapper {
    ...
    @Select({"<script>select ID, CODE, FSRQ, DWJZ from FUND_JZ WHERE 1=1 <if test=\"code!=null and code!=''\"> and CODE = #{code} </if> and FSRQ >= date('now', '-20 days') order by FSRQ ASC limit 20 </script>" })
